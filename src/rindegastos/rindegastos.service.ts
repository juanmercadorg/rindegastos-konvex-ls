...
// y hace el arreglo para enviarlos como entradas diarías (asientos contables) a SAP
  async arrangeEntries(dateSince: Date, reportId: string) {
    ...
    var comentarioAsientos = "FXR " + report['ReportNumber'] + " " + report['EmployeeName'];

    //Si el tipo de rendición es caja chica entonces el crédito se irá a la cuenta de Caja Chica

    if (String(reportType) == 'CCH'){
      
      journalEntryLines.push( { AccountCode: '11101000', Debit: "0", Credit: report['ReportTotalApproved'].toString() , LineMemo: "FXR " +report['ReportNumber'] + " " + report['EmployeeName'] + " Proyecto-"+ report['ExtraFields'].find(field => field.Name === "Área")?.Value, ProjectCode: report['ExtraFields'].find(field => field.Name === "Área")?.Code});
    ...
    for (var expense of expenses['Expenses']) {
            ...
            if (String(tipoDoc) !== "33" && String(tipoDoc) !== "99"){
              ...
              journalEntryLines.push(
                { AccountCode: expense.CategoryCode, Debit: expense.OriginalAmount.toString(), Credit: "0" , LineMemo: "FXR " + report['ReportNumber'] + " " +report['EmployeeName'] + "- " + expense.Note, CostingCode: expense.ExtraFields.find(field => field.Name === "Centro de costo")?.Code, ProjectCode: ReportProjectCode})
            ...
            console.log(rutLimpio)
              ...
              journalEntryLines.push(
                { ShortName: rutLimpio, Debit: expense.OriginalAmount.toString(), Credit: "0" , LineMemo: "FXR "+ report['ReportNumber'] + " "+ report['EmployeeName'] + "- " + expense.Note, CostingCode: expense.ExtraFields.find(field => field.Name === "Centro de costo")?.Code, ProjectCode: ReportProjectCode});
...
async findAllReports(dateSince: string, dateUntil: string): Promise<ExpenseReport[]> {

  let DateSince = dateSince;
  let DateUntil = dateUntil;
    try{
        const config: AxiosRequestConfig = {
          method: 'get',
          maxBodyLength: Infinity,
          //Solo se consultan informes cerrados que no esten integrados
          url: `https://api.rindegastos.com/v1/getExpenseReports?Since=${DateSince}&Until=${DateUntil}`,
          headers: {
            'Authorization': 'Bearer '+ process.env.RG_APIKEY,
            'Content-Type': 'application/json'
          }
      ...
      console.log("hola, ocurrió un error al obtener los informes en Rindegastos: " + errorMessage )
      console.log("antes de enviar el mensaje");
      const op = await this.setIntegrationMessage( "Ocurrió un error al intentar obtener los informes en Rindegastos", "Error al integrar")
      console.log("después de enviar el mensaje", op);
      return error
    }
